/**
 * Script to add a 1-month subscription for a vendor
 * Usage: node scripts/addVendorSubscription.js <user_id> [package_id]
 * Example: node scripts/addVendorSubscription.js 9074135121
 */
...
async function addVendorSubscription(userId, packageId = null) {
  try {
    console.log(`\n🔄 Adding 1-month subscription for vendor ${userId}...\n`);
    ...
    const userInvoices = await Invoice.findByUserId(userId);
    ...
    try {
      ...
      if (allShops && allShops.length > 0) {
        ...
        console.log(`⚠️  No shop found for user ${userId}`);
      }
    } catch (shopError) {
    ...
    console.log(`\n✅ Successfully added 1-month subscription for vendor ${userId}`);
...
if (!userId) {
  console.error('❌ Usage: node scripts/addVendorSubscription.js <user_id> [package_id]');
  console.error('   Example: node scripts/addVendorSubscription.js 9074135121');
  process.exit(1);
}

addVendorSubscription(userId, packageId);
