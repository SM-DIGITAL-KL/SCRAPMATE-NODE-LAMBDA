...
/**
 * Parse all user data from SQL
 */
function parseAllUsers(sqlContent) {
  const items = [];
  
  // Find all INSERT INTO statements for users table
  const insertRegex = /INSERT INTO `users`[^;]+;/gis;
  ...
  for (const match of matches) {
    ...
    const colMatch = match.match(/INSERT INTO `users`\s*\(([^)]+)\)/i);
    ...
    // Parse rows - handle nested parentheses and multiline
    let currentRow = '';
    let depth = 0;
    let inString = false;
    let stringChar = null;
          ...
          if (depth === 1) {
            ...
            if (rowValues.length === columns.length) {
              ...
              columns.forEach((col, index) => {
                ...
                if (col === 'id' || col === 'mob_num' || col === 'contact' || col === 'language' || col === 'del_status') {
 ...
 */
async function getDynamoDBUsers() {
...
async function main() {
  ...
  console.log(`\n📋 Parsing ALL user data from SQL...`);
  const sqlItems = parseAllUsers(sqlContent);
  console.log(`  📊 SQL file has: ${sqlItems.length} users`);
  
  // Get unique IDs
  const sqlIds = new Set(sqlItems.map(item => String(item.id)).filter(id => id && id !== 'null'));
  console.log(`  📊 Unique user IDs in SQL: ${sqlIds.size}`);
  
  console.log(`\n🔍 Checking DynamoDB...`);
  const dynamoItems = await getDynamoDBUsers();
  console.log(`  📊 DynamoDB has: ${dynamoItems.length} users`);
  
  // Get DynamoDB IDs
  const dynamoIds = new Set(dynamoItems.map(item => String(item.id)).filter(id => id && id !== 'null'));
  console.log(`  📊 Unique user IDs in DynamoDB: ${dynamoIds.size}`);
  ...
  console.log(`\n🔎 Missing user IDs: ${missingIds.length}`);
  ...
  missingItems.forEach((item, index) => {
    console.log(`  ${index + 1}. ID: ${item.id}, Name: ${item.name || 'N/A'}, Mobile: ${item.mob_num || 'N/A'}, Email: ${item.email || 'N/A'}, Type: ${item.user_type || 'N/A'}`);
  });
  ...
  console.log(`\n📦 Importing ${missingItems.length} missing users...`);
  const client = getDynamoDBClient();
  const batchSize = 25;
  let imported = 0;
    ...
    try {
      ...
      for (const item of batch) {
        try {
          ...
          imported++;
          console.log(`    ✅ Imported user ID: ${item.id} (${item.name || 'N/A'})`);
        } catch (putError) {
          console.error(`    ❌ Failed to import user ID ${item.id || 'unknown'}:`, putError.message);
        }
      }
    }
  }
  ...
  const finalItems = await getDynamoDBUsers();
  const finalIds = new Set(finalItems.map(item => String(item.id)).filter(id => id && id !== 'null'));
  
  console.log(`\n✅ Final Results:`);
  console.log(`   SQL users: ${sqlItems.length} (${sqlIds.size} unique IDs)`);
  console.log(`   DynamoDB users: ${finalItems.length} (${finalIds.size} unique IDs)`);
