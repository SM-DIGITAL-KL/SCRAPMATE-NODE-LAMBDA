...
# Add utility routes to API Gateway
# Usage: ./scripts/add-utility-routes.sh [api-name] [stage] [region]
...
echo "🚀 Adding Utility Routes to API Gateway"
...
if [ -z "$INTEGRATION_ID" ]; then
    ...
    aws lambda add-permission \
        ...
        --statement-id "api-gateway-utility-$(date +%s)" \
...
# Utility routes from services/utility/routes.js
UTILITY_ROUTES=(
    "POST /api/get_table"
...
echo ""
echo "📋 Creating utility routes..."
SUCCESS=0
FAILED=0

for ROUTE_KEY in "${UTILITY_ROUTES[@]}"; do
...
FINAL_ENDPOINT="${API_ENDPOINT}/api"
...
echo ""
echo "🧪 Test Endpoints:"
echo "   curl $FINAL_ENDPOINT/get_all_tables -H 'api-key: zyubkfzeumeoviaqzcsrvfwdzbiwnlnn'"
echo "   curl $FINAL_ENDPOINT/count_row/users -H 'api-key: zyubkfzeumeoviaqzcsrvfwdzbiwnlnn'"
echo ""
